        ...
        for (char move : MOVESET) {
            ...
            if (terrain == '.') {
                ...
                if (curr.changed.count(new_pt) > 0) {
                    ...
                    pstate temp = {new_pt, curr.bomb_count+1, curr.path+move, -1*(taxi_dist(new_pt,dst)*.10),curr.changed};
                ...
                if (curr.changed.count(new_pt) > 0) {
                    ...
                    pstate temp = {new_pt, curr.bomb_count-1, curr.path+move, -1*(taxi_dist(new_pt,dst)*2), curr.changed};
                    temp.changed.insert(new_pt);
                    explorer.push(temp);
                }
            } else {
